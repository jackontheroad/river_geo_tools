...
# file_url = r"d:\geo\guijiang_final\guijiang_river_osm_sorted_dissolved.shp"
# file_url = r"d:\geo\xiangjiang\xiangjiang_river_osm_sorted_dissolved.shp"
kml_path_url = r"d:\geo\changjiang\changjiang_river_osm_sorted_dissolved.kml"
...
def parse_kml(kml_file):
    ...
    coordinates = []
    first_placemark_name = None
    for placemark in root.findall('.//{http://www.opengis.net/kml/2.2}Placemark'):
        linestring = placemark.find('.//{http://www.opengis.net/kml/2.2}LineString')
        if first_placemark_name is None:
            first_placemark_name = placemark.find('.//{http://www.opengis.net/kml/2.2}name').text.strip()
        if linestring is not None:
            coords_str = linestring.find('.//{http://www.opengis.net/kml/2.2}coordinates').text.strip()
            # coords = [coord.split(',')[:2] for coord in coords_str.split()]
            coords = [list(map(float, coord.split(',')[:2])) for coord in coords_str.split()]
            coordinates.extend(coords)

    return first_placemark_name, coords_str, coordinates

def create_dataframe(coordinates):
    df = pd.DataFrame(coordinates, columns=['lon', 'lat'])
    return df

first_placemark_name, coords_str, coordinates = parse_kml(kml_path_url)

# 创建DataFrame
route_df = create_dataframe(coordinates)
...
# 添加样式
kml_content += f'''
<name>{first_placemark_name}</name>
<Style id="red_icon">
    <IconStyle>
        <color>ff0000ff</color>  <!-- Red color -->
    </IconStyle>
</Style>
<Folder id="PointsFolder">
    <name>Points</name>
'''
# 解析每行内容并添加到KML内容中
for index, row in milestones_df.iterrows():
    ...
    lon = row['lon']
    lat = row['lat']
    coords = str(lon) + "," + str(lat)  # 最后一个作为coords
    kml_content += f'''
    <Placemark>
        <name>{name}</name>
        <styleUrl>#red_icon</styleUrl>
        <Point>
...
'''
base, ext = os.path.splitext(kml_path_url)
# 将KML内容写入输出文件
with open("%s_with_milestones_%0.2fKM.kml" % (base, distance / 1000), 'w', encoding='utf-8') as f:
        f.write(kml_content)
